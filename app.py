...
async def read_file(path: str = Query(..., description="File path to read")):
    ...
    Ensures file access is restricted to /data directory (B1).
    """
    try:
        ...
        if not validate_file_path(path):
            raise HTTPException(
                status_code=403, 
                detail="Access denied. Can only read files from the data directory."
            )
        ...
        if not os.path.isfile(abs_path):
            raise HTTPException(
                status_code=400,
                detail="Path must point to a file"
            )
